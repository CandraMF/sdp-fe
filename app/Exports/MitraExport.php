...
class MitraExport implements FromArray, WithCustomStartCell, ShouldAutoSize, WithTitle, WithStyles //,WithDrawings
{
    ...
    public function array(): array
    {
        ...
        $valueSheet[] = [""];
        $valueSheet[] = ["No", "Nama Mitra", "Penanggung Jawab", "Alamat", "No Telpon", "Provinsi", "Kota"];

        //data
        foreach ($this->data as $index => $item) {
            $no = $index + 1;
            $valueSheet[] = [$no, $item['nama_mitra'], $item['nama_pic'], $item['alamat'], $item['no_telp'], $item['propinsi'], $item['kabkot']];
        }
