...
class PesertaPelatihanKeterampilanService
{
	...
	public function search(array $data, $url = null)
	{
		...
		$q = $q->leftJoin(DB::raw("(SELECT ppk.id_wbp, 1 as checked_val FROM peserta_pelatihan_keterampilan AS ppk INNER JOIN daftar_peserta_pelatihan_keterampilan AS dppk ON ppk.id_daftar_peserta_pelatihan_keterampilan=dppk.id WHERE dppk.id_jadwal_pk='" . $id_jadwal_pk . "') as ppk"), 'identitas.id', '=', 'ppk.id_wbp');
