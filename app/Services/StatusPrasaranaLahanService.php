...
class StatusPrasaranaLahanService
{
	...
	public function search(array $data, $url = null)
	{
		...
		$defaultColumn = ['status_prasarana_lahan.id', 'prasarana_ruang.nama_prasarana_ruang', 'status_prasarana_lahan.status', 'status_prasarana_lahan.kepemilikan', 'status_prasarana_lahan.luas_dipakai', 'status_prasarana_lahan.lahan_tidur', 'status_prasarana_lahan.foto'];
		$q = StatusPrasaranaLahan::query();
		$q = $q->select($defaultColumn);
		$q = $q->join('prasarana_ruang', 'status_prasarana_lahan.id_prasarana_lahan', '=', 'prasarana_ruang.id');
