...
class MitraService
{
    ...
    public function search(array $data, $url = null)
    {
        ...
        $defaultColumn = ["id_mitra", "nama_mitra", "nama_pic", "alamat", "no_telp", "provinsi.deskripsi as propinsi", "dati2.deskripsi as kabkot"];
        $q = Mitra::query();
        $q = $q->select($defaultColumn);
        $q = $q->leftJoin('dati2', 'mitra.id_dati2', '=', 'dati2.id_dati2');
        $q = $q->leftJoin('provinsi', 'dati2.id_provinsi', '=', 'provinsi.id_provinsi');
        $data = $this->mapping($q);
    ...
    public function printPDF($data)
    {
        ...
        $columnOfValues = ['nama_mitra', 'nama_pic', 'alamat', 'no_telp', 'propinsi', 'kabkot'];
        $sizeCellcolumns = ["Nama Mitra" => 1000, "Penanggung Jawab" => 1000, "Alamat" => 1500, "No Telpon" => 500, "Provinsi" => 1000, "Kota" => 1000];
        $sizeCells = ['nama_mitra' => 1000, 'nama_pic' => 1000, 'alamat' => 1500, 'no_telp' => 500, 'propinsi' => 1000, 'kabkot' => 1000];
        ...
        $namaFileWord = 'Referensi-Mitra-' . Carbon::now()->format('Ymdhis') . '.doc';
        $templateProcessor->saveAs(Storage_path('temp/word/' . $namaFileWord));


        $dataFile["namaFilePdf"] = 'Referensi-Mitra-' . Carbon::now()->format('Ymdhis') . '.pdf';
